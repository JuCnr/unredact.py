            ...
            for block in text_dict["blocks"]:
                for line in block.get("lines", []):
                    for span in line.get("spans", []):
                        if span["text"].strip():
# On vérifie si le span est sous une redaction (rectangle noir ou blanc)
                            span_rect = fitz.Rect(span["bbox"])  # rectangle du texte
                            is_hidden = False
                            for path in page.get_drawings():
                                rect = path["rect"]
                                fill_color = path.get("fill")
                                stroke_color = path.get("color")
                                # Si c'est une boîte noire ou blanche suffisamment grande
                                if rect.intersects(span_rect):
                                    if fill_color and (all(c < 0.05 for c in fill_color) or all(c > 0.95 for c in fill_color)):
                                        is_hidden = True
                                        break
                                    if stroke_color and all(c < 0.05 for c in stroke_color) and path.get("width", 0) > 10:
                                        is_hidden = True
                                        break

# Appliquer la couleur
        text_color = (1, 0, 0) if is_hidden else (0, 0, 0)
